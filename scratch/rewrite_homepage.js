...
import { 
  User, Clock, ArrowRight, Play, Pause, ChevronLeft, ChevronRight, Calendar, Volume2, BookOpen, Brain, Sparkles, PenTool, Users, Eye, MessageSquare
} from "lucide-react";
...
export default function Home() {
  const { articles, magazines, settings, ads, homepageSections } = useCms();

  const publishedArticles = articles.filter(a => a.status === "Published" || a.status === "Approved" || !a.status);
  
  // Basic renderer for dynamic sections
  const renderSection = (section: any) => {
    ...
    // Filter articles based on config
    const sectionArticles = publishedArticles.filter(a => a.category === category);
    ...
    switch (section.section_type) {
        ...
        return (
          <section key={section.id} className="max-w-7xl mx-auto px-4 md:px-8 py-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              {sectionArticles.slice(0, 1).map((article) => (
      ...
      case "ad":
        const activeAds = ads.filter(a => a.status === 'Active' && a.placement_id === config.placement_id);
        ...
        return (
          <section key={section.id} className="max-w-7xl mx-auto px-4 md:px-8 py-6 flex justify-center">
             <a href={adToDisplay.target_url} target="_blank" rel="noopener noreferrer" className="block w-full max-w-4xl bg-slate-100 dark:bg-slate-800 rounded-xl overflow-hidden shadow-sm border border-slate-200 dark:border-slate-700 relative group">
                ...
                <img src={adToDisplay.image_url} alt={adToDisplay.title} className="w-full h-auto object-cover opacity-90 group-hover:opacity-100 transition-opacity" />
             </a>
          </section>
        );
          ...
          <section key={section.id} className="max-w-7xl mx-auto px-4 md:px-8 py-8 bg-slate-50 dark:bg-slate-900/50 rounded-3xl my-8 border border-slate-200 dark:border-slate-800">
            <div className="flex flex-col md:flex-row gap-8 items-center">
              <div className="w-full md:w-1/3 flex justify-center">
                <img src={latestMag.coverImage} alt={latestMag.issue} className="w-48 h-auto shadow-xl rounded-md border-4 border-white dark:border-slate-800" />
              </div>
    ...
    <div className="w-full min-h-screen bg-white dark:bg-[#0A0F1D] pb-16">
      ...
      {homepageSections && homepageSections.length > 0 ? (
        ...
        <div className="flex items-center justify-center py-20 text-slate-500">
          <p>No homepage sections configured. Please configure them in the CMS.</p>
        </div>
      )}
    </div>
  );
...
console.log('Homepage rewritten to be 100% dynamic!');
