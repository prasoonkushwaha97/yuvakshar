...
DECLARE
    ...
    v_exists BOOLEAN;
BEGIN
    -- Create clean base name from display name or email prefix
    v_base := COALESCE(p_display_name, SPLIT_PART(p_email, '@', 1));
    
    -- Lowercase, replace non-allowed characters with hyphens
    v_base := LOWER(v_base);
    v_base := REGEXP_REPLACE(v_base, '[^a-z0-9_.-]', '-', 'g');
    -- Deduplicate hyphens/underscores/dots
    v_base := REGEXP_REPLACE(v_base, '[-_.]+', '-', 'g');
    -- Strip leading/trailing hyphen/dot/underscores
    v_base := REGEXP_REPLACE(v_base, '^[-_.]+|[-_.]+$', '', 'g');
    
    -- Enforce length constraint for base
    ...
    -- Prevent reserved usernames as base
