...
import ChaupalDiscoverySidebar from "@/components/chaupal/layout/ChaupalDiscoverySidebar";
...
export default function ChaupalLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] dark:bg-[#090D16]">
      <div className="max-w-[1400px] mx-auto flex justify-center gap-6 lg:gap-8 px-0 sm:px-4 lg:px-8">
        
        {/* Left Navigation */}
        <ChaupalDesktopSidebar />

        {/* Main Feed / Content Area */}
        <main className="w-full max-w-[640px] xl:max-w-[720px] flex-1 pb-24 md:pb-8 border-x border-slate-200 dark:border-slate-800 bg-white dark:bg-[#0F172A] min-h-screen shadow-sm shadow-slate-200/20 dark:shadow-none">
          <Suspense fallback={
