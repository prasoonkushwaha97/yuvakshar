...
export default function FollowingPage() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      if (match) {
        setProfile(match);
        // Find actual profiles from the following list
        const followingIds = match.following || [];
        const followingProfiles = users.filter((u: Profile) => followingIds.includes(u.id));
  ...
  return (
    <div className="max-w-[600px] mx-auto w-full bg-white dark:bg-[#070B14] rounded-2xl border border-slate-200 dark:border-slate-800 min-h-[80vh]">
      ...
      <div className="flex border-b border-slate-200 dark:border-slate-800">
        <Link href={`/community/u/${username}/followers`} className="flex-1 text-center py-3 text-slate-500 hover:bg-slate-50 dark:hover:bg-slate-900 transition-colors font-hindi text-sm">फ़ॉलोअर्स</Link>
        <div className="flex-1 text-center py-3 border-b-2 border-primary font-bold text-slate-900 dark:text-white font-hindi text-sm">फ़ॉलोइंग</div>
      </div>
      ...
      <div className="divide-y divide-slate-100 dark:divide-slate-800/50">
        {filteredFollowing.length > 0 ? (
          ...
          <div className="py-20 text-center text-slate-500 font-hindi">यह उपयोगकर्ता अभी किसी को फ़ॉलो नहीं कर रहा है।</div>
        )}
      </div>
    </div>
  );
