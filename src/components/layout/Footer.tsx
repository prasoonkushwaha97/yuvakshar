...
export default function Footer() {
  ...
  const handleSubscribe = async (e: React.FormEvent) => {
    ...
    setMsg(res);
    // Do not clear the email if logged in to maintain autofill, or clear it. The task says:
    // "Auto-fill email state when user logs in"
    // So let's clear it or keep it. Let's clear it on success but we can leave it.
    setEmail("");
    setTimeout(() => setMsg(""), 4000);
  };

  return (
    <footer className="w-full bg-[#FAF8F3] dark:bg-[#0A0F1D] text-[#1E1E1E] dark:text-slate-300 border-t border-[#E7E2D8] dark:border-slate-800 py-4 lg:py-6 mt-6 lg:mt-8 text-xs relative z-30">
      <div className="max-w-7xl mx-auto px-4 md:px-8 grid grid-cols-1 md:grid-cols-12 gap-3 md:gap-6">
        
        {/* Left Area: Brand & Tagline */}
        <div className="md:col-span-3 space-y-1.5 md:space-y-2.5">
          ...
          <p className="text-slate-600 dark:text-slate-400 font-serif leading-relaxed">
            {settings.general.tagline} — लेखन, चिंतन और परिवर्तन। युवाओं, विचारकों और रचनाकारों का हिन्दी डिजिटल साझा मंच।
          </p>
        </div>
        ...
        <div className="md:col-span-3 space-y-1.5 md:space-y-2.5">
          ...
          <div className="flex flex-col space-y-2.5 text-slate-600 dark:text-slate-400 font-medium">
        ...
        <div className="md:col-span-3 space-y-1.5 md:space-y-2.5">
          ...
          <div className="flex flex-col space-y-2.5 text-slate-600 dark:text-slate-400 font-medium">
            ...
            {/* Social Media Links */}
            <div className="flex space-x-3 text-slate-500 dark:text-slate-400 pt-2">
        ...
        <div className="md:col-span-3 space-y-1.5 md:space-y-2.5">
          ...
          <div className="p-3 bg-white dark:bg-[#0F172A]/50 rounded-2xl border border-[#E7E2D8] dark:border-slate-800 shadow-sm space-y-2">
            <p className="text-slate-600 dark:text-slate-400 leading-relaxed text-[11px]">
              साक साप्ताहिक विमर्श, रचनाएं और नए संस्करणों की सूचना सीधे अपने ईमेल इनबॉक्स में प्राप्त करें।
            </p>
            <form onSubmit={handleSubscribe} className="flex space-x-2">
              <input 
                ...
                className="flex-grow bg-[#F5F2EB] dark:bg-slate-900 border border-[#E7E2D8] dark:border-slate-800 text-[11px] px-3 py-2 rounded-xl focus:outline-none focus:border-primary text-slate-800 dark:text-slate-200"
                required
              />
              <button 
                type="submit" 
                className="bg-[#EA580C] hover:bg-[#EA580C]/90 text-white p-2.5 rounded-xl transition-all shadow-sm shrink-0 cursor-pointer flex items-center justify-center"
              >
                <Send className="w-3.5 h-3.5" />
              </button>
            </form>
            {msg && <p className="text-[10px] text-primary font-medium animate-pulse">{msg}</p>}
          </div>
        </div>
